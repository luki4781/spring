	...
	<table style="width: 80%">
		...
		<tbody>
			<c:forEach var="obj" items="${data }">
				<tr>
					<td>${obj.PNAME } <small>(<fmt:formatNumber
								value="${obj.GAB }" pattern="#,##0.00" />일 남음)
					</small></td>
					<td><fmt:formatNumber value="${obj.SPRICE }" pattern="#,###" /></td>
					<td><fmt:formatNumber value="${obj.IPRICE }" pattern="#,###" /></td>
					<td><c:if test="${!empty obj.CDATE }">~<fmt:formatDate
								value="${obj.CDATE }" pattern="yyyy.MM.dd" />
						</c:if></td>
				</tr>
			</c:forEach>
		</tbody>
